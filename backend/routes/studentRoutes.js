...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    //   res.json(students);
    // } else {
    //   res.status(404).json({ message: 'No student found with the given information.' });
    // }
    res.json(students);
  })
);
...
//following route is for updating the students details
router.put(
  '/update/:id', 
  protect,
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      existingStudent.student_name = student_name;
      existingStudent.email = email;
      existingStudent.address = address;
      existingStudent.gender = gender;
      existingStudent.classname = classname;
      existingStudent.contact_no = contact_no;
      existingStudent.parents_name = parents_name;
      existingStudent.age = age;
      existingStudent.registration_fees = registration_fees;
